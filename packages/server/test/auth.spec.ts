...
import { AuthController, CSRF_SECRET } from "../src/controller/auth/auth-controller"
...
describe("Social Login", () => {
    ...
    it("Should able to login", async () => {
        ...
        const result = await controller.facebook(stub.facebook({ id: user.socialId }), new Token().create(CSRF_SECRET))
    ...
    it("Should able to register and login", async () => {
        const controller = new AuthController()
        const result = await controller.facebook(stub.facebook({ id: "12345678" }), new Token().create(CSRF_SECRET))
        const login = getLoginUserFromCallback(result)
